...
pub mod hints {
    ...
    use spin::Mutex;
    pub static INV_NONDET: Lazy<Mutex<BTreeMap<BaseElement, BaseElement>>> =
        Lazy::new(|| Mutex::new(BTreeMap::new()));

    pub static INV_NONDET_QUAD: Lazy<Mutex<BTreeMap<[BaseElement; 2], [BaseElement; 2]>>> =
        Lazy::new(|| Mutex::new(BTreeMap::new()));
}
...
impl FieldElement for BaseElement {
    ...
    fn inv(self) -> Self {
        #[cfg(feature = "use-hints")]
        {
            ...
            if let Some(res) = INV_NONDET.lock().get(&self) {
                let res_c = res.clone();
                assert!(
                    res_c * self == BaseElement::ONE,
                    "res_c = {}, self = {}",
                    res_c,
                    self
                );
        ...
        #[cfg(all(feature = "generate-hints", feature = "std"))]
        {
            ...
            INV_NONDET.lock().insert(self, res);
...
impl ExtensibleField<2> for BaseElement {
    ...
    fn use_hint(a: [Self; 2]) -> Option<[Self; 2]> {
        ...
        {
            // means we are running as part of the verifier
            if let Some(res) = INV_NONDET_QUAD.lock().get(&a) {
                let res_c = res.clone();
                return Some(res_c);
            } else {
                return None;
    ...
    fn save_hint(a: [Self; 2], b: [Self; 2]) -> () {
        ...
        {
            // means we are running as part of the prover
            INV_NONDET_QUAD.lock().insert(a, b);
